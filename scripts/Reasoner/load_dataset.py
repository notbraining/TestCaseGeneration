...
if __name__ == "__main__":
    ...
    def isValid(test_in: str, test_out: str):
        test = [test_in.strip(), test_out.strip()]

        if len(test[0]) > 20 or len(test[1]) > 10:
            return False

        for i in range(2):
            for num in test[i].split():
                if not num.isdigit():
                    continue
                if int(num) > 25:
                    return False
    ...
    def make_parquet(split):
        def make_format(question: str, inputs: list[str], outputs: list[str]):
            filtered_inputs, filtered_outputs = map(
                list, zip(*list(filter(remove_long, zip(inputs, outputs))))
            )

            prompts = []
            ground_truth = []
            for input, output in zip(filtered_inputs, filtered_outputs):
        ...
        prompts = []
        ground_truth = []
        print("formatting prompts + groundtruth and filtering out large test cases...")
